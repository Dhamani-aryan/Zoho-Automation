...
(
  ...
  'Selector map from KD Blitz playbook section 9: Compose Email button is a button with text Compose Email. To input #ceToAddr_1; To chips [id^="ceToAddrDetails"] li.selectedEmail; remove stale chips via .closeIconB. Subject #ceSubject_1. Cc link is small text Cc; Cc input #ceCCAddr_1; Cc chips [id^="ceCCAddrDetails"]. Body editor is iframe frame_selector #z_editor with inner #editorDiv and signature #ecw_signature. Insert body nodes immediately before #ecw_signature as Verdana, Geneva, sans-serif at 13.33px; no leading blank line; two blank lines before signature. Never assign #editorDiv innerHTML/textContent, call replaceChildren, or use ui_step fill_field on the whole editor because those remove the existing signature. Use CDP trusted input and a real Enter key to commit To and each CC chip. Schedule button is text Schedule, last visible match width < 160; popup time #schTimeMail with zero-padded option like 08:00 PM; date #startDate or #bstDate; confirm button text Schedule & Close.',

...
(
  ...
  'Each item needs enough identity to resolve one Contact and related Deal, plus subject, body, schedule date/time, and an explicit CC list. A blank CC means no CC. For imports/samples/KD Blitz Batch 3 All Contacts Email Drafts.md only, parse its header: IT/Technical gets Email A, Finance gets Email B, Leadership gets Email A; use subject option 1; CC prashant.sharma@klouddata.com and ankur@klouddata.com; schedule time is 8:00 PM; body boundary is Best, and Zoho signature stays below. Those values never apply to another file. The only permitted question for that file is the TBD schedule date.',
  'Use schedule_zoho_email_batch first for structured email scheduling. It resolves Contact to Account to Deal from Supabase; duplicate-checks, creates, completes, and API-read-back verifies requested Tasks against the exact Deal and Contact; then opens the direct Potential URL, composes and verifies exact To/CC/Subject/body/signature, schedules once, and verifies Scheduled without model calls between records. Task preparation uses the logged-in Zoho API transport, not the Open Activities dialog. Use browser_eval/ui_step only for one focused email-composer recovery after the deterministic tool returns a specific recoverable failure. Schedule never send immediately. For undo, scheduled emails are non-revertible in scope; document manual path: open the related deal, Emails, Scheduled tab, find the matching recipient/subject/time, and use Zoho UI delete/cancel manually.',
  $guide$Navigation recovery: if browser_observe shows /tab/Home, a list, or the wrong record while the deal id/URL is known, call ui_step open_url with https://crm.zoho.com/crm/org890324941/tab/Potentials/{dealId}, wait for the expected deal identity, and continue. Home alone is not a stop condition.
...
Never assign #editorDiv innerHTML/textContent, call replaceChildren, or use ui_step fill_field on the whole editor.$guide$,
  ...
  'Before scheduling: read browser_observe.composer for exact To and Cc chips, subject, body_text, and signature_present. This compact summary survives a truncated control observation. If a required value is absent, perform one targeted read-only browser_eval using window.document for top composer fields and frame_selector #z_editor for body/signature before reporting verification impossible. The composer-fill eval must return a JSON object containing exact To and Cc chip values, subject, body text/spacing, and signature_present=true; if it returns no value, observe/read back before retrying because the composer may already be changed. Screenshot evidence should show correct recipient(s), first subject, Hi {FirstName} with no blank line above, two blank lines before signature. After scheduling: confirm success toast "Your mail has been scheduled successfully" and verify the email appears in the Scheduled tab at the requested date/time. Report counts, failures, screenshots/evidence ids, and non-revertible scheduled-email undo notes.',
  'Stop on missing email/subject/body/date/time, unknown or ambiguous target record, wrong deal/contact/account after verified navigation, duplicate scheduled email, To/Cc chip mismatch, composer/iframe still not found after one known-record navigation and re-observation, logged out, immediate-send risk, or Scheduled-tab verification mismatch.',
...
(
  ...
  'Use the authenticated Zoho Tasks API path. Read Deals/{id}/Activities_Chronological_View for exact open-task matches, POST Tasks with Subject, Due_Date, What_Id, Who_Id, and $se_module=Deals, or PUT the exact Task id with Status=Completed. Never guess among duplicate subjects.',
  'UI is not the normal task write path. The deterministic email scheduler performs task API operations inside its existing approved task-order job, then uses browser UI only for email compose and schedule.',
  'Zoho Tasks are a first-class API module. What_Id must read back as the expected Deal, Who_Id as the expected Contact when supplied, and completed tasks must appear in Activities_Chronological_View_History.',
  'Read back Tasks/{id} after every create/update. Verify exact Subject, Due_Date when created, Status, What_Id, and Who_Id; also verify completed tasks in Deal activity history.',

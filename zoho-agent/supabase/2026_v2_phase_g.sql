...
(
  ...
  'Use schedule_zoho_email_batch first for structured email scheduling. It resolves Contact to Account to Deal from Supabase, opens the direct Potential URL, composes and verifies exact To/CC/Subject/body/signature, schedules once, and verifies Scheduled without model calls between records. Use browser_eval/ui_step only for one focused recovery after the deterministic tool returns a specific recoverable failure. Schedule never send immediately. For undo, scheduled emails are non-revertible in scope; document manual path: open the related deal, Emails, Scheduled tab, find the matching recipient/subject/time, and use Zoho UI delete/cancel manually.',

...
- When the user attaches or references a Claude-written email draft Markdown file, parse the batch rules, direct-link fields, email body, CC, subject, new tasks, closed tasks, and each contact section. Resolve TO_RESOLVE or missing Zoho contact/deal/account/task links yourself using Supabase mirror search first and live Zoho when current identity matters. Use contact email as the strongest key, then contact name + account/company + deal name. Do not ask the user for links, tool choices, or selectors. Stop only for true ambiguity, missing required recipient/body/subject, or a missing schedule date/time the draft/request does not specify.
- For every structured email scheduling request, call schedule_zoho_email_batch after parsing the complete input. Do not manually reproduce its compose/schedule phases with browser_eval, browser_observe, or ui_step unless the deterministic tool returns a specific recoverable failure and one focused recovery is justified. For an attached file or more than 3 records, propose one task order first, then call the batch tool once with all email records and complete the order from its per-record report.

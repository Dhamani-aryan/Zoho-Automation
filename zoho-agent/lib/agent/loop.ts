...
- Email composer recipient reconciliation method: identify To/Cc chips only by the email attribute of [id^="ceToAddrDetails"] li.selectedEmail or [id^="ceCCAddrDetails"] li.selectedEmail, with case-insensitive email comparison; never compare visible label text. Before typing anything, read existing chips. If a pre-filled chip email already equals the resolved recipient, keep it and type nothing. If it differs, remove it through browser_input action "remove" using the chip text/selector or the observed removable item, then add the correct address. When the user says remove/cancel/delete a visible chip, tag, pill, recipient, or selected item, do not click the item as a focusing step; call browser_input action "remove" directly. Do not rely on Backspace unless browser_input action "key" is given a selector/text to focus first. After any Enter commit, poll the chip list for up to about 5 seconds until no chip is unresolved: unresolved means label "Loading", missing/empty email attribute, or a pending/loading class. Never judge chips while one is unresolved. If the same email appears in more than one chip, remove extras through browser_input action "remove" and re-verify; this is deduplication, not ambiguity. Stop only when, after resolution, a chip email attribute differs from the target set, or leftover input text is non-empty after a clear attempt. Same rules apply for CC. A blank CC in the current draft means no CC.
...
- If a browser_input click returns an offscreen/not-clickable error, do not retry the same click; re-observe, then either use browser_input remove for chip/token removal or actuate the control with a browser_eval element click, and prefer zoho_api for any non-composer action.

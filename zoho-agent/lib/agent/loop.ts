...
- Call economy: batch observation and serialize commitment. One read-only browser_eval should return the full bundle needed for next actions, including To/Cc chips with email attributes, leftover address inputs, Cc/Bcc presence, subject value, body/signature state, and schedule control rect. Resolve record sets in one mirror search/query per module when possible, not one query per record. Commits such as chip Enter, Schedule click, and Schedule & Close stay one-at-a-time and each is verified. Target the one-email-two-task run at 10-14 tool calls.

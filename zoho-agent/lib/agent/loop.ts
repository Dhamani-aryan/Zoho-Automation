...
- Schedule popup method: observe the live composer bottom controls before clicking; the Schedule control is near Send, but selectors are hints and must be confirmed against the DOM at action time. After opening Schedule, browser_observe returns schedule_popup read-back (date, time, hour, minute, ampm, timezone). The visible time field is a filtered combobox whose typed text never commits on its own; browser_input type on the schedule time field (#schTimeMail) or date field (#startDate) commits deterministically into Zoho's backing fields and returns verified read-back - use it with values like "1:00 PM" or "Jul 14, 2026" and treat verified=false as failure. Post-midnight times roll to the next calendar day. Confirm with "Schedule & Close", then verify the popup closed and the email appears in the record Emails -> Scheduled list or the internal scheduled-mail read-back. Never assume a memorized selector or coordinate is still valid without live observation.

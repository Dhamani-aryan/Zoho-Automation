...
- When the user attaches or references a Claude-written email draft Markdown file, parse the header rules and each contact section. Resolve TO_RESOLVE or missing Zoho contact/deal/account links yourself using Supabase mirror search first and live Zoho when current identity matters. Use contact email as the strongest key, then contact name + account/company + deal name. Do not ask the user for links, tool choices, or selectors. Stop only for true ambiguity, missing required recipient/body/subject, or the TBD schedule date.

...
export const BROWSER_TOOL_DEFINITIONS: AgentToolDefinition[] = [
  {
    ...
    tier: 1,
    description:
      "Capture the current crm.zoho.com page as a ranked interactive snapshot. Returns stable @eN refs with roles, accessible names, primary/fallback selectors, frame scope, state, and geometry, plus composer and schedule_popup read-back blocks when those surfaces are open. Composer chrome (recipient inputs, chips, Cc/Bcc reveal links) is boosted in ranking, and chip remove controls that only render on hover are included as refs flagged hidden_until_hover=true (browser_input hovers the chip automatically for those). Use those refs with browser_input. Pass target_selector or target_text for extra local descendants, siblings, hit targets, and pseudo-element content. Read-only and ungated.",
    parameters: {
      type: "object",

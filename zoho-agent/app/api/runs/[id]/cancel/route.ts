...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!canManageRun(controlRun, auth.user)) {
      return NextResponse.json({ error: "You cannot cancel another user's run." }, { status: 403 });
    }

    assertRunTransition(controlRun.status as RunStatus, "cancelled");
    ...
    const { data: updated, error: updateError } = await auth.supabase
      ...
      .eq("id", controlRun.id)
      .select("id,status,completed_at,stop_reason")
      .single();

    if (updateError || !updated) {
      return NextResponse.json({ error: updateError?.message ?? "Run could not be cancelled." }, { status: 500 });
    }
    ...
    await writeRunAudit({
      ...
      message: "Cancelled workflow run.",
      metadata: { cancelled_at: cancelledAt }
    });

    return NextResponse.json({ run: updated });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Run cancel failed unexpectedly.";
    console.error("[run-cancel]", message, error);
